...
public InvoiceFrame(int clientId, String clientName, String clientPhone, int userId, int invoiceHeaderId) {
    ...
    // Pass the invoiceHeaderId here, we'll load full object inside loadInvoiceData
...
private void setupUI() {
    ...
    // Financial Info Panel (centered row of 3 sections)
    JPanel financialRow = new JPanel();
    financialRow.setLayout(new FlowLayout(FlowLayout.CENTER, 20, 10));
    financialRow.setComponentOrientation(ComponentOrientation.RIGHT_TO_LEFT);

    // ----------- Discount Panel -----------
    JLabel discountLabel = new JLabel("قيمة الخصم:");
    ...
    discountText.setFont(new Font("Segoe UI", Font.PLAIN, 16));
    discountText.setHorizontalAlignment(JTextField.CENTER);
    discountText.getDocument().addDocumentListener(new DocumentListener() {
    ...
    JPanel discountPanel = new JPanel(new BorderLayout(5, 2));
    discountPanel.setBorder(BorderFactory.createTitledBorder("الخصم"));
    discountPanel.setComponentOrientation(ComponentOrientation.RIGHT_TO_LEFT);
    discountPanel.setPreferredSize(new Dimension(200, 80));

    JPanel discountLabelPanel = new JPanel(new FlowLayout(FlowLayout.CENTER));
    discountLabelPanel.add(discountLabel);

    discountPanel.add(discountLabelPanel, BorderLayout.NORTH);
    discountPanel.add(discountText, BorderLayout.CENTER);

    // ----------- Income Panel -----------
    JLabel incomeLabel = new JLabel("المبلغ الذي تم دفعه (العميل):");
    incomeLabel.setFont(new Font("Segoe UI", Font.BOLD, 14));
    incomeLabel.setForeground(new Color(0, 102, 204));
    ...
    JPanel incomePanel = new JPanel(new BorderLayout(5, 2));
    incomePanel.setBorder(BorderFactory.createTitledBorder("المدفوع"));
    incomePanel.setComponentOrientation(ComponentOrientation.RIGHT_TO_LEFT);
    incomePanel.setPreferredSize(new Dimension(200, 80));

    JPanel incomeLabelPanel = new JPanel(new FlowLayout(FlowLayout.CENTER));
    incomeLabelPanel.add(incomeLabel);

    incomePanel.add(incomeLabelPanel, BorderLayout.NORTH);
    incomePanel.add(incomeField, BorderLayout.CENTER);

    // ----------- Payment Method Panel -----------
    JLabel payMethodLabel = new JLabel("طريقة الدفع:");
    payMethodLabel.setFont(new Font("Segoe UI", Font.BOLD, 14));
    payMethodLabel.setHorizontalAlignment(SwingConstants.CENTER);

    payComboBox = new JComboBox(paymentMethodsChoicesArabic);
    payComboBox.setPreferredSize(new Dimension(150, 25));
    payComboBox.setFont(new Font("Segoe UI", Font.BOLD, 14));

    JPanel payPanel = new JPanel(new BorderLayout(5, 2));
    payPanel.setBorder(BorderFactory.createTitledBorder("طريقة الدفع"));
    payPanel.setComponentOrientation(ComponentOrientation.RIGHT_TO_LEFT);
    payPanel.setPreferredSize(new Dimension(200, 80));

    JPanel payLabelPanel = new JPanel(new FlowLayout(FlowLayout.CENTER));
    payLabelPanel.add(payMethodLabel);

    payPanel.add(payLabelPanel, BorderLayout.NORTH);
    payPanel.add(payComboBox, BorderLayout.CENTER);

    // Add them right-to-left
    financialRow.add(discountPanel);
    financialRow.add(incomePanel);
    financialRow.add(payPanel);

    // ----------- Total Panel -----------
    totalLabel.setText("الإجمالي: 0.00");
totalLabel.setFont(new Font("Segoe UI", Font.BOLD, 16));
totalLabel.setForeground(new Color(0, 153, 0));     totalLabel.setHorizontalAlignment(SwingConstants.CENTER);

    netTotalLabel = new JLabel("0.00 :صافي الإجمالي");
    netTotalLabel.setFont(new Font("Segoe UI", Font.BOLD, 14));
    netTotalLabel.setHorizontalAlignment(SwingConstants.CENTER);

    JPanel totalPanel = new JPanel(new FlowLayout(FlowLayout.CENTER, 10, 0));
    totalPanel.setComponentOrientation(ComponentOrientation.RIGHT_TO_LEFT);
    totalPanel.add(totalLabel);
    totalPanel.add(netTotalLabel);
    ...
    // Compose bottom
    JPanel financialContainer = new JPanel(new BorderLayout(0, 10));
    financialContainer.add(financialRow, BorderLayout.NORTH);
    financialContainer.add(totalPanel, BorderLayout.CENTER);

    bottomPanel.add(financialContainer, BorderLayout.NORTH);
...
private void updateTotalAmount() {
    ...
    // Read discount field and subtract from total
    try {
        discountValue = Float.parseFloat(discountText.getText().trim());
    } catch (NumberFormatException ex) {
        discountValue = 0.0f;
    }

    float netTotal = totalAmount - discountValue;
    if (netTotal < 0) netTotal = 0;

    // Update UI labels
    totalLabel.setText(String.format("%.2f :الإجمالي", totalAmount));
    netTotalLabel.setText(String.format("%.2f :صافي الإجمالي", netTotal));
// if (incomeField.isEditable()) {
//        incomeField.setText(String.format("%.2f", netTotal));
    //}
}
    ...
    try {
        ...
        // Fetch invoice header first (to get discount, total, remaining)
        InvoiceHeader invoiceHeader = invoiceHeaderDao.findById(invoiceHeaderId);
        if (invoiceHeader == null) {
            statusLabel.setText("لم يتم العثور على بيانات الفاتورة");
            return;
        }
        ...
        if (details.isEmpty()) {
            ...
            netTotalLabel.setText("0.00 :صافي الإجمالي");
            return;
        }
        ...
        // Use the discount stored in invoice header (fixed value)
        float discount = invoiceHeader.getDiscount();
        discountText.setText(String.format("%.2f", discount));

        // Show total amount (sum of details)
        totalLabel.setText(String.format("%.2f :الإجمالي", totalAmount));

        // Calculate net total = totalAmount - discount (don't let it go negative)
        double netTotal = totalAmount - discount;
        if (netTotal < 0) netTotal = 0;

        netTotalLabel.setText(String.format("%.2f :صافي الإجمالي", netTotal));

        statusLabel.setText("تم تحميل بيانات الفاتورة بنجاح");
incomeField.setText(String.format("%.2f", invoiceHeader.getIncome()));
...
private void deleteInvoice() {
    ...
    private void saveInvoiceData() {
        ...
        try {
            ...
            // Read and validate discount
float discount;
try {
    discount = Float.parseFloat(discountText.getText().trim());
    if (discount < 0) {
        showMessage("الخصم لا يمكن أن يكون سالبًا", true);
        return;
    }
    if (discount > totalInvoiceAmount) {
        showMessage("الخصم لا يمكن أن يتجاوز المبلغ الإجمالي", true);
        return;
    }
} catch (NumberFormatException e) {
    showMessage("قيمة الخصم غير صالحة", true);
    return;
}

// Recalculate total after discount
float discountedTotal = totalInvoiceAmount - discount;

// Validate income against discounted total
if (income > discountedTotal) {
    showMessage("الدفع لا يمكن أن يتجاوز المبلغ بعد الخصم (" + discountedTotal + ")", true);
    return;
}

            float remaining = totalInvoiceAmount - income;
            float first_income=income;
...
//PaymentMethod = selected;
            ...
            InvoiceHeader invoiceHeader = new InvoiceHeader(
                    invoiceHeaderId, clientId, totalInvoiceAmount, income,first_income, remaining,discount,selected,
                    currentTime, currentTime
            );
invoiceHeader.setFirstIncome(income); 
